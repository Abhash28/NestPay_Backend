...
const addProperty=async(req,res,next)=>{
    ...
    try {
       ...
       res.status(200).json({success:true,message:"Property  & unit Save Successfully",Property})
    } catch (error) {
        next(error)
    }
}
...
const fetchAllProperty=async(req,res,next)=>{
    ...
    try {
        ...
        res.status(200).json({success:true,count:allProperty.length,message:"Fetching all Property",allProperty})
    } catch (error) {
        next(error)
    }
}
